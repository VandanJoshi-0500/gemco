        ...
        <div class="card-body d-sm-flex d-block  align-items-center p-lg-3 p-2 user_header ">
            ...
            <div class="ms-auto">
                <div class="d-flex align-items-center">
                    <a href="{{route('admin.add.user')}}" class="btn gc_btn align-items-center d-none d-md-flex"><span class="fs-4 me-2">+</span>Add New User</a>
                </div>
            </div>
        </div>
    </div>
                ...
                <tbody>
                    @if(isset($users) && $users->isNotEmpty())
                        @foreach ($users as $user)
                            <tr>
                                <td><a href="{{route('admin.edit.user',$user->id)}}">{{$loop->index+1}}</a></td>
                                <td data-header="First Name" class="pt-2">
                                    {{$user->first_name}}
                                </td>
                                <td data-header="Last Name" class="pt-2">
                                    {{$user->last_name}}
                                </td>
                                ...
                                <td data-header="Action" class="gc_flex">
                                    <div class="d-flex align-items-center justify-content-end">
                                        <a href="{{route('admin.edit.user',$user->id)}}"><i class="ri-edit-2-fill fs-20"></i></a>
                                        <a href="javascript:void(0);" data-id="{{ $user->id }}" class="ms-2 delete-btn"><i class="ri-delete-bin-fill fs-20"></i></a>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
